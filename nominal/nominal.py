...
E192 = [1.00, 1.01, 1.02, 1.04, 1.05, 1.06, 1.07, 1.09, 1.10, 1.11, 1.13, 1.15,
        1.17, 1.18, 1.20, 1.20, 1.21, 1.23, 1.24, 1.26, 1.27, 1.29, 1.30, 1.32,
...
def find_values(ratio, standard=E192):
    norm = math.pow(10, math.floor(math.log10(ratio)))
    nratio = float(ratio) / norm
    ...
    def find_nearest(x):
        ...
        while (j - i > 1):
            middle = (i + j) // 2
            if standard[middle] < x:
                i = middle
            else:
                j = middle
        a = standard[i]
        b = (j < len(standard)) and standard[j] or 10.0
        return [a, b]

    for b in standard:
        ideal = b * nratio
        aa = find_nearest(ideal)
        for a in aa:
            found.append((a * norm, b))

    found.sort(key = lambda (a, b) : abs(a/b - ratio))

    result = [ ]
    err = 0
    for (a, b) in found:
        e = abs(a/b - ratio)
        if err < ratio/10000: err = e
        if e > err * 3: break
        result.append((a, b))
...
def print_values(ratio, standard=E192):
    for a, b in find_values(ratio ,standard):
        print a, b, abs(a/b - ratio)/ratio*100
